    ...
    public class MaxHeap<T> where T : IComparable<T>
    {
        ...
        public T Pop()
        {
            ...
            if(_elements.Count > 0)
            {
                ...
                if (_elements.Count > 1 && lastElement.CompareTo(toReturn) < 0)
                    SiftDown();
            }
        ...
        private void SiftDown()
        {
            for(int i = 0; i < _elements.Count; i++)
            {
                ...
                if(2 * i + 1 < _elements.Count && _elements[2 * i + 1].CompareTo(parent) > 0)
                {
                    child = _elements[2 * i + 1];
                    _elements[2 * i + 1] = parent;
                    _elements[i] = child;
                    SiftDown();
                } else if(2 * i + 2 < _elements.Count && _elements[2 * i + 2].CompareTo(parent) > 0)
                {
