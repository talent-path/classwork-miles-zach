...
export class AddressFormComponent {
  ...
  get verifyButtonDisabled(): boolean {
    return !(this.address && this.city && this.state && this.zip && !this.valid);
  }

  async verifyAddress(): Promise<any> {
    let message = "Address could not be verified.";
    let style = 'error-snackbar';
    const promise = await fetch(`https://us-street.api.smartystreets.com/street-address?street=${this.address}&city=${this.city}&state=${this.state}&zipcode=${this.zip}&match=invalid&key=101457761082301642&auth-id=7e7776c9-0bc2-eb79-3434-b174b3dc622b&auth-token=jLpYZmAm448dp2K7rffN`)
      .then(res => res.json())
